 ...
 * If fd is >= 0, use that file descriptor instead of opening a new one
 */
int send_file_contents(int sockfd, int fd)
{
#if USE_AESD_CHAR_DEVICE
    int should_close = 0;
    ...
    return 0;
#else
    (void)fd;  // Unused parameter when not using char device
    pthread_mutex_lock(&g_file_mutex);
 ...
 */
int handle_seekto_command(int sockfd, const char *packet)
{
#if USE_AESD_CHAR_DEVICE
    ...
    int fd;
    int ret = -1;
    char *packet_copy = NULL;
    char *newline_pos = NULL;
    
    // Remove newline if present (packet includes newline from receive_packet)
    ...
    newline_pos = strchr(packet_copy, '\n');
    if (newline_pos) {
        *newline_pos = '\0';
    }
    
    // Parse the command: AESDCHAR_IOCSEEKTO:X,Y
    if (sscanf(packet_copy, "AESDCHAR_IOCSEEKTO:%u,%u", &write_cmd, &write_cmd_offset) != 2) {
        syslog(LOG_ERR, "Failed to parse seekto command: %s", packet_copy);
        free(packet_copy);
        return -1;
    }
    ...
    // Read from device using the same file descriptor (file offset is honored)
    // Read in chunks
    char buffer[BUFFER_SIZE];
    ssize_t bytes_read;
...
void *handle_client_thread(void *arg)
{
    ...
    while (!g_signal_received) {
        ...
        // Check if this is a seekto command
        if (strncmp(packet, "AESDCHAR_IOCSEEKTO:", 18) == 0) {
            // Handle seekto command specially
            if (handle_seekto_command(client_fd, packet) != 0) {
                free(packet);
                break;
            }
            free(packet);
        ...
        if (send_file_contents(client_fd, -1) != 0) {
            free(packet);
            break;
        }
